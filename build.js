 ...
 * this repo's adr/0002-multilingual-pt-es.md records the local deviations).
 * English is authoritative and hand-written; es/pt are machine-translated from
 * committed caches (data/i18n/<lang>.json, managed by scripts/translate.js —
 * never hand-edit) and carry a visible disclaimer.
 *
 * The language is a path segment AFTER the project (/rcc/{en,es,pt}/…) because
 ...
 * institutional arc, diffusion timeline, contested-numbers chart — is covered
...
// Page paths (relative to a locale root) the site emits. rcc is a single-page
...
// (lineage.edgeLegend) — an addition to the template's key set, to be ported
// back to core/template (adopt-template rule 6).
...
const UI = {
  en: {
    ...
    // Contested-numbers chart chrome. The per-series axis note and caption say
    // WHO reported each number and IN WHAT UNIT — the figure's whole point is
    // that the series are not comparable, so this chrome has to be as clear in
    // pt/es as it is in English.
...
function siteBase(meta) {
  const raw = (meta && meta.siteUrl) || 'https://cronologia.github.io/rcc/';
  return raw.replace(/\/+$/, '') + '/';
}
 ...
 * subtrees (references, approvalLadder) translate only their allowlisted keys
 * via SUBTREE_TRANSLATABLE. With an empty dictionary (English) the values are
 * unchanged, so the render stays byte-identical to a pre-i18n build.
 */
...
${cells ? `            ${cells}\n` : ''}            <td class="sw-total">${lane.total}</td>
  ...
  // NOTE: the template uses renderText() here so a basis can carry [[term-id]]
  // glossary markers. This repo has no glossary machinery (no renderText, no
  // vendored glossary-terms.json), so bases are plain escaped prose.
  const bases = layout.lanes
    .map((lane) => `            <li><strong>${esc(lane.label)}</strong> — ${esc(lane.basis || '')}${renderCites(lane.sources, refNumById)}</li>`)
    .join('\n');
...
${placesMapHtml}${branchTimelineHtml}${lineageHtml}${numbersChartHtml}    <section id="figures">
...
module.exports = {
  ...
  renderPage,
  LOCALES, ROUTES, OG_LOCALE, UI, TRANSLATABLE_KEYS, SUBTREE_TRANSLATABLE, keysFor,
  UNKNOWN_REF_TYPES, renderReference, loadDict, siteBase, translator, localizeData,
