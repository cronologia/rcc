...
function deriveUserAgent(projectName) {
  const raw = (typeof projectName === 'string' && projectName.trim()) ? projectName.trim() : 'RCC Cronologia project';
  const name = headerSafe(raw) || 'RCC Cronologia project';
  return `cronologia-check-links/1.0 (${name}; +https://github.com/cronologia/rcc)`;
}
// <<< ADOPT
