...
test('deriveUserAgent names the project and has a contact URL', () => {
  assert.match(deriveUserAgent('RCC Brasil'), /^cronologia-check-links\/1\.0 \(RCC Brasil; \+https:\/\/github\.com\/cronologia\/rcc\)$/);
  assert.match(deriveUserAgent(''), /RCC Cronologia project/);
  assert.match(deriveUserAgent(undefined), /RCC Cronologia project/);
});
