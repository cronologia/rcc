...
data/i18n/{pt,es}.json    GENERATED machine-translation caches (English source string
...
scripts/validate-data.js  Schema check (runs in CI before the build); warns on place
                          strings missing from the gazetteer, never fails on them
scripts/sync-places.js    Out-of-band: refresh data/places.json from cronologia/core
                          (sibling checkout or network); --check detects a stale copy
...
scripts/translate.js      Translation-cache manager: `--stats` reports pt/es coverage;
                          offline-safe (no backend needed), re-author when English changes
build.js                  Compiler: data/chronology.json (+ archives.json + i18n caches)
...
1. **Edit data, not output.** Change `data/chronology.json`, run
   ...
   `docs/`, `data/archives.json` and `data/i18n/*.json` are generated — never
   hand-edit them. When you change English prose, re-author the `pt`/`es`
   caches too (`node scripts/translate.js --stats` shows what is missing);
   an untranslated string falls back to English rather than failing the build,
   so stale locales are silent unless you look. English is authoritative.
